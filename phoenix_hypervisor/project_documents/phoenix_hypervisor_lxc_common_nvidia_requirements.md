# `phoenix_hypervisor_lxc_nvidia.sh` - Detailed Requirements

## Overview

This document outlines the detailed requirements for the `phoenix_hypervisor_lxc_nvidia.sh` script. This script configures NVIDIA GPU support inside a specific LXC container by passing through host devices and installing the necessary driver and CUDA toolkit.
...
*   **Content:**
    ...
    *   (Optional) Validate the format of `GPU_ASSIGNMENT` against the pattern `^(none|[0-9]+(,[0-9]+)*)$`.
