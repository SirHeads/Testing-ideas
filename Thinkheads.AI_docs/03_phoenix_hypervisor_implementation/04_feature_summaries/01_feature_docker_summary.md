...
summary: The `docker` feature is a foundational component of the declarative stack management system. It automates the installation and configuration of the Docker engine within a VM, preparing it to receive and run declarative Docker stacks.
...
---
The `docker` feature is a prerequisite for any VM that will be used to run Docker workloads. It installs the Docker engine and prepares the VM to be managed by the Portainer API.
...
This feature is applied to any VM that is intended to run Docker workloads. It is a prerequisite for the declarative stack management system and is typically listed in the `features` array in `phoenix_vm_configs.json`.
