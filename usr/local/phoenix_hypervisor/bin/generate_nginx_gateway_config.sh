...
CERT_FILE="/mnt/pve/quickOS/lxc-persistent-data/101/ssl/nginx.${DOMAIN_NAME}.crt"
KEY_FILE="/mnt/pve/quickOS/lxc-persistent-data/101/ssl/nginx.${DOMAIN_NAME}.key"

log_info "Verifying existence of Nginx SSL certificate and key..."
if [ ! -f "$CERT_FILE" ]; then
    log_fatal "Nginx SSL certificate not found at: $CERT_FILE. Please ensure certificates are generated before running sync."
fi

if [ ! -f "$KEY_FILE" ]; then
    log_fatal "Nginx SSL key not found at: $KEY_FILE. Please ensure certificates are generated before running sync."
fi
log_success "Nginx SSL certificate and key found."
...
server {
    ...
    ssl_certificate_key /etc/nginx/ssl/nginx.$DOMAIN_NAME.key;
